...
# The ability to modify lists is particularly important when you're working with a list of users on a websire or a list of characters ina  game
...
# For example, if we have a rectangle that should always be a certain size, we can ensure that its size doesn't change by putting the dimentions into a tuple
dimensions = (200,50)
print(dimensions[0])
print(dimensions[1])
# We define the tuple dimensions using parentheses instead of square brackets
# We print each element in the tuple individually
...
# We are trying to alter a tuple, which can't be done to that type of object, Python tells us we can't assign a new value to an item in a tuple
...
# We can redefine the entire tuple
dimensions = (200,50)
